...
val df = spark.read.option("header", true).option("inferSchema", "true").option("delimiter", ";").csv("/home/guadalupe/Escritorio/BigData/Unidad4.scala/bank-full.csv")
...
// Indexar todas las variables para despues arrojar las columnas set in & set out
